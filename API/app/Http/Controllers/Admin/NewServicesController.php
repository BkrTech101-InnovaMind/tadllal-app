...
class NewServicesController extends Controller
{
    ...
    {
        $orders = NewServicesOrders::with('user', 'NewServices')->get();

        return $this->success([
            'orders' => NewServicesOrdersResource::collection($orders),
        ]);
    }
    ...
    {
        $order = NewServicesOrders::find($orderId);
    ...
    {
        $approvedOrders = NewServicesOrders::where('status', 'Approved')->with('user', 'subConstructionService')->get();

        return $this->success([
            'approvedOrders' => NewServicesOrdersResource::collection($approvedOrders),
        ]);
    }

    public function getPendingServiceOrders()
    {
        $pendingOrders = NewServicesOrders::where('status', 'Under Review')->with('user', 'subConstructionService')->get();

        return $this->success([
            'pendingOrders' => NewServicesOrdersResource::collection($pendingOrders),
        ]);
    }
