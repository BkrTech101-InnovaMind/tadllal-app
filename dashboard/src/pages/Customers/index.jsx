...
import { countMatchingItems, tableFilters } from "@/api/filtersData"
...
export default function index() {
  ...
  async function fetchCustomers() {
    ...
    try {
      const customersData = await api.get(
        "customerRequests/getAllCustomerRequests",
        authToken
      )
  ...
  const handleDropdownChange = async (e, itemId, type) => {
    let endpoint =
      type == "Request" ? "editRequestStatus" : "editCommunicationStatus"
    ...
    try {
      ...
      await api.put(
        `customerRequests/${endpoint}/${itemId}`,
        encodedData,
        authToken
      )
      ...
      const updatedCustomers = customers.map((customer) => {
        if (customer.id === itemId) {
          return type == "Request"
            ? {
                ...customer,
                attributes: { ...customer.attributes, request_status: e },
              }
            : {
                ...customer,
                attributes: { ...customer.attributes, communication_status: e },
              }
        }
        return customer
      })
  ...
  const handleStatusSelect = (selectedStatues, type) => {
    const filterField =
      type == "Request" ? "request_status" : "communication_status"
      ...
      render: (item) => (
        <div className='flex items-center'>
          <select
            ...
            onChange={(e) =>
              handleDropdownChange(e.target.value, item.id, "Request")
            }
          >
      ...
      render: (item) => (
        <div className='flex items-center'>
          <select
            ...
            onChange={(e) =>
              handleDropdownChange(e.target.value, item.id, "communication")
            }
          >
                ...
                <div className='flex w-full '>
                  <DropDownList
                    ...
                    onSelect={(selectedStatus) =>
                      handleStatusSelect(selectedStatus, "Request")
                    }
                  />
                  <DropDownList
                    ...
                    onSelect={(selectedStatus) =>
                      handleStatusSelect(selectedStatus, "communication")
                    }
                  />
                </div>
